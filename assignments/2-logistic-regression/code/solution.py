...
def logistic_regression(data, label, max_iter, learning_rate):
    ...
    while (iter < max_iter):
		 #compute gradient
        g=np.zeros((1,features))
        for index, value in enumerate(label):
            numerator = value * data[index] 
            temp=0
            for i in range(features):
                temp=temp+w[0,i]*data[index,i]
            denominator = (1 + np.exp(value * temp))
...
def thirdorder(data):
    ...
    '''
    transform=np.zeros((data.shape[0],10))
    for index, value in enumerate(data):
        transform[index,0]=1
        transform[index,1]=value[0]
        transform[index,2]=value[1]
        transform[index,3]=value[0]*value[0]
        transform[index,4]=value[0]*value[1]
        transform[index,5]=value[1]*value[1]
        transform[index,6]=value[0]*value[0]*value[0]
        transform[index,7]=value[0]*value[0]*value[1]
        transform[index,8]=value[0]*value[1]*value[1]
        transform[index,9]=value[1]*value[1]*value[1]
    return transform 
...
def accuracy(x, y, w):
    ...
    for index, value in enumerate(x):
        temp=0
        for i in range(features):
            temp = temp + value[i]*w[0,i]
        prob = np.exp(temp)/(1+np.exp(temp))
        classification=-1
        if (prob > 0.5):
            classification=1
